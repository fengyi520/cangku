...
type ImportContextAction = ImportFixAction | CreateStyleAction | MapRowsToStyleAction | CreateInboundDraftFromJobAction;
...
class AiChatService {
  ...
  async chat(user: AuthUser, message: string, jobId?: string) {
    ...
    const prompt = [
      ...
      `权限：查看库存=${canViewInventory ? "是" : "否"}；创建入库草稿=${canCreateDraft ? "是" : "否"}；新建商品=${canManageCatalog ? "是" : "否"}。`,
    ...
    let parsed: { reply?: string; action?: { type?: string; lines?: Array<{ skuId?: string; quantity?: number }>; fixes?: Array<{ rowId?: string; patch?: Record<string, unknown> }>; jobId?: string; styleNo?: string; name?: string; brand?: string | null; category?: string | null; variants?: Array<{ skuCode?: string; color?: string; size?: string; minStock?: number }>; rows?: Array<{ row?: number; color?: string; size?: string }> } | null };
    ...
    if (parsed.action?.type === "create_inbound_draft") {
      ...
      // 兜底：AI 逐行生成可能漏行。若当前是识别任务上下文且任务所有行均校验通过、
