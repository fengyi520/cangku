...
type AiChatInboundDraftAction = { type: "create_inbound_draft"; warehouseId: string; lines: Array<{ skuId: string; skuCode: string; styleNo: string; color: string; size: string; quantity: number }> };
...
type AiChatAction = AiChatInboundDraftAction | AiChatFixRowsAction | AiChatCreateStyleAction;
type AiChatPreview = { action: AiChatInboundDraftAction; previewToken: string; rows: Array<{ skuId: string; skuCode: string; styleNo: string; color: string; size: string; quantity: number; currentOnHand: number; projectedOnHand: number; errors: string[]; warnings: string[] }>; totals: { quantity: number; delta: number }; valid: boolean; expiresAt: string };
...
function AiChatPage({ user }: { user: User }) {
  ...
  const send = useMutation({ mutationFn: (message: string) => api<{ reply: string; action: AiChatAction | null; permissions: { canViewInventory: boolean; canCreateDraft: boolean; canManageCatalog: boolean } }>("/ai/chat", { method: "POST", body: jsonBody(selectedJob ? { message, jobId: selectedJob } : { message }) }), onSuccess: (result) => setMessages((current) => [...current, { role: "assistant", text: result.reply, action: result.action }]), onError: (error) => setMessages((current) => [...current, { role: "assistant", text: errorText(error) }]) });
